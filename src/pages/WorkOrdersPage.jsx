import React, { useState, useMemo } from 'react';
    ...
    const WorkOrdersPage = () => {
      ...
      const { work_orders = [], customers = [], vehicles = [], employees = [] } = data;
                  ...
                  filteredOrders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="text-right">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => openForm(order)}>
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => confirmDelete(order.id)}>
